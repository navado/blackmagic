...
bool nrf51_probe(target *t)
{
	...
	/* Check that page_size and ram_size makes sense */
	...
	uint32_t info_part = target_mem_read32(t, NRF52_PART_INFO) & 0xFFFF;
	if ((info_part != 0xffff) && (info_part != 0)) { 
		uint32_t ram_size = target_mem_read32(t, NRF52_INFO_RAM);
		t->idcode = info_part;
		t->driver = "Nordic nRF52";
		target_add_ram(t, 0x20000000, ram_size * 1024);
		nrf51_add_flash(t, NRF51_UICR,
						page_size * code_size,
						page_size);
